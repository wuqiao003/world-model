#!/usr/bin/env python3
"""
K2v4: iso-fact / asymmetric suite so pixel metrics can fail.

Adds pos_only: FiLM trained only on actions with ang>0.
On mixed/negated CF planning it should underperform film despite
competitive on-policy fact_err — the regime where gated CAF / dual should help.
"""
...
OUT = Path(os.environ.get("OUT_DIR", "runs/k2v4_isofact"))
...
LR = 3e-4
HPLAN = 5
TAU = float(os.environ.get("FACT_TAU", "1.25"))
...
def make_traj(n, scale=1.0, ang_sign=None):
    frames = [torch.randn(n, 3, H, W, device=DEVICE) * 0.2]
    acts = scale * torch.randn(n, T - 1, A, device=DEVICE)
    if ang_sign is not None:
        # force ang component sign (index 2)
        acts[..., 2] = ang_sign * acts[..., 2].abs()
    for t in range(T - 1):
...
def train(model, steps=STEPS, scale=1.0, ang_sign=None):
    opt = torch.optim.AdamW(model.parameters(), lr=LR)
    model.train()
    for _ in range(steps):
        fr, ac = make_traj(BATCH, scale=scale, ang_sign=ang_sign)
...
def metrics(model, n=80) -> Dict[str, float]:
    model.eval()
    # on-policy-ish mixed eval
    fr, ac = make_traj(n, scale=1.0)
    pred = model(fr[:, :-1], ac)
...
def cf_plan_err(model, n=48) -> float:
    """Downstream: mostly negated / large actions (CF-sensitive)."""
    model.eval()
    fr, _ = make_traj(n, scale=1.0)
    acts = 1.5 * torch.randn(n, HPLAN, A, device=DEVICE)
    acts[..., 2] = -acts[..., 2].abs()  # all negative ang — hits pos_only OOD
    cur = fr[:, 0]
    preds = []
    for t in range(HPLAN):
        ...
        preds.append(pred)
        cur = pred
    return F.mse_loss(torch.stack(preds, 1), oracle_roll(fr[:, 0], acts)[:, 1:]).item()
...
def main():
    ...
    film_best_util = min(names, key=lambda n: plan[n]) == "film"
    ours_keys = ("caf_gated", "score_iv", "dual")
    film_top = any(argmax_metric(k) == "film" for k in ours_keys)
    best_ours = max(corr[f"spearman_{k}"] for k in ours_keys)
    # pass: ours beats raw caf AND beats or ties neg_fact within 0.05 OR film tops gated while fact mis-ranks
    fact_ranks_film = argmax_metric("neg_fact") == "film"
    pass_k2 = (
        film_best_util
        and film_top
        and corr["spearman_caf"] < best_ours
        and (best_ours >= corr["spearman_neg_fact"] - 0.05 or not fact_ranks_film)
    )
    ...
    out = {
        ...
        "cf_plan_errors": plan,
        "correlations": corr,
        "film_best_util": film_best_util,
        "film_top_by_gated": film_top,
        "fact_ranks_film": fact_ranks_film,
        "pass_k2v4": pass_k2,
        "note": "pos_only asymmetric; CF plan uses negative ang",
    }
